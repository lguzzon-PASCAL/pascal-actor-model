...
Begin
	...
	Send(TLogDebugMessage.Create(ccMainThreadName, ccSwitchboardName, 'Received new line.'));
End;
...
Begin
	...
	// Lets use a logger actor to log all that is happening
	TLoggerActor.Create.Start;
	Repeat
		SwitchBoard.Mailbox.Push(TLogDebugMessage.Create(ccMainThreadName, ccSwitchboardName, 'Reading a new line.'));
		// Reads a new line
		ReadLn(lLine);
		// If the line instructs us to quit, do it
		If lLine = 'quit' Then
		Begin
			SwitchBoard.Mailbox.Push(TLogDebugMessage.Create(ccMainThreadName, ccSwitchboardName, 'User asked to quit.'));
		...
		Begin
			SwitchBoard.Mailbox.Push(TLogDebugMessage.Create(ccMainThreadName, ccSwitchboardName, 'User typed a normal line.'));
			// Creates the message
			...
			SwitchBoard.Mailbox.Push(TLogDebugMessage.Create(ccMainThreadName, ccSwitchboardName, 'Message sent to consumer.'));
		End;
	Until lLine = 'quit';
